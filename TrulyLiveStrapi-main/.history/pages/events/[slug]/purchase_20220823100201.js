...
export async function getServerSideProps(context) {
  ...
  var mainUrl = context.req.headers.referer
  if (mainUrl.indexOf("?")!=-1) mainUrl=mainUrl.substring(0,mainUrl.indexOf("?"))
  if (mainUrl.indexOf("/p")!=-1) mainUrl=mainUrl.substring(0,mainUrl.indexOf("?"))
//  console.log(context.req.headers)
  ...
  try {
    ...
    console.log("Success URL: "+mainUrl+'/purchasedone?session={CHECKOUT_SESSION_ID}')
    console.log("Session created. URL: "+session.url)
  } catch(e){
    throw(e.message)
  }

  console.log("Now diverting to "+session.url)
