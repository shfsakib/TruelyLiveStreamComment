...
function Navbar({ navData, className }) {
  ...
  useEffect(() => {
    ...
    logoutRef.current.style.display = token?'':'none';
    accountRef.current.style.display = token?'':'none';
    watchingRef.current.style.display = ( token && user?.isAdmin ) ?'':'none';
    watchRef.current.style.display = token?'':'none';
